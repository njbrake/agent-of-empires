...
describe("applyEvent / UserPromptSent", () => {
  ...
  it("appends successive ToolCallStreamChunk frames into the partial buffer (#1075)", () => {
    let state = emptyCockpitState();
    state = applyEvent(state, {
      session_id: "s-1",
      seq: 1,
    ...
    expect(state.toolOutputs["tc-bash"]).toBe(
      "first line\nsecond line\nthird line\n",
    );
  });

  it("ToolCallCompleted drains the stream-chunk buffer (#1075)", () => {
    let state = emptyCockpitState();
    state = applyEvent(state, {
      session_id: "s-1",
      seq: 1,
    ...
    expect(state.toolOutputs["tc-bash"]).toBeUndefined();
    const done = state.activity.find((a) => a.id === "done-tc-bash");
    expect(done!.text).toBe("streamed body\n");
  });
