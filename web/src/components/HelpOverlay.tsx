...
export function HelpOverlay({ onClose }: Props) {
  return (
    ...
    >
      <div
        className="bg-surface-800 border border-surface-700 rounded-lg w-[480px] max-w-[90vw] shadow-2xl"
      ...
      >
        ...
        <div className="p-5">
          <div className="mb-5">
            <h3 className="font-mono text-[11px] uppercase tracking-widest text-slate-500 mb-2">
              Dashboard
            </h3>
            <div className="space-y-1">
              {SHORTCUTS.map((s) => (
                <div key={s.key} className="flex items-center gap-3">
                  <kbd className="font-mono text-[12px] bg-surface-900 border border-surface-700 rounded px-1.5 py-0.5 text-brand-500 min-w-[32px] text-center">
                    {s.key}
                  </kbd>
          ...
          <div>
            ...
            <div className="space-y-1">
              {TERMINAL_SHORTCUTS.map((s) => (
                <div key={s.key} className="flex items-center gap-3">
                  <kbd className="font-mono text-[12px] bg-surface-900 border border-surface-700 rounded px-1.5 py-0.5 text-accent-600 min-w-[32px] text-center">
                    {s.key}
                  </kbd>
                  <span className="font-body text-sm text-slate-400">
                    {s.desc}
