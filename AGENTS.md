...
- **No emdashes or `--`** as separators in docs/comments; use commas, semicolons, or rephrase. Exception: `docs/cli/reference.md` is generated by `cargo xtask gen-docs` and inherits clap-markdown's em-dash bullet separator. That's a renderer choice applied uniformly to every entry, not human-authored prose, so leave it alone.
