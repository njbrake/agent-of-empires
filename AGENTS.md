...
The public website (agent-of-empires.com) is an Astro static site in `website/`. All markdown content has a **single source of truth**:

- **`docs/`** is the canonical source for all documentation and guide content. Edit docs here, never on the website side.
- **`website/src/pages/guides/*.md`** and **`website/src/pages/docs/**/*.md`** are auto-generated at build time by `website/scripts/sync-docs.mjs`. These files are `.gitignored`; never edit or commit them.
- The sync script strips the `# Title` line, rewrites relative `.md` links to website URLs (or GitHub links for unpublished pages), rewrites `assets/` image paths to `/assets/`, and prepends Astro frontmatter (layout, title, description).
- Frontmatter metadata (title, description) and the source-to-dest mapping for each page live in the `PAGES` array inside `website/scripts/sync-docs.mjs`. The `URL_MAP` in the same file controls link rewriting.
- Astro component pages (`*.astro`) like `website/src/pages/guides/index.astro` are NOT generated; edit them directly.
- `docs/cli/reference.md` is auto-generated by `cargo xtask gen-docs`. The sync script then copies it to the website. So the chain is: clap help text → `cargo xtask gen-docs` → `docs/cli/reference.md` → sync script → website.
