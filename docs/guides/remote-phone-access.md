...
- **Idle logout after 24 hours.** Every authenticated request slides the session deadline 24 hours into the future. An actively-used device stays logged in indefinitely; one whole day with no requests from the device invalidates the session and the next visit hits the passphrase prompt again. Same model as an SSH connection that drops once you stop using it.
