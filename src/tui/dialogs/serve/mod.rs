//! Serve / remote-access dialog.
//!
//! Owns the multi-phase state machine (mode picker → confirm → starting →
//! active → error), the daemon process management, the log tail, and the
//! rendering for each phase. Big file because the phases are tightly
//! intertwined — the passphrase generator was the one self-contained piece
//! and lives in `passphrase`.
