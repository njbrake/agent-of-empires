...
/// Stub for agents whose status is managed entirely by hooks.
pub fn hook_managed_status(_content: &str) -> Status {
    Status::Idle
}
...
mod tests {
    ...
    #[test]
    fn test_detect_status_from_content_unknown_tool_defaults_to_idle() {
    ...
    #[test]
    fn test_hook_managed_status_always_idle() {
        assert_eq!(hook_managed_status("anything here"), Status::Idle);
        assert_eq!(hook_managed_status("esc to interrupt"), Status::Idle);
        assert_eq!(hook_managed_status("Do you want to proceed?"), Status::Idle);
    }
}
