    ...
    if cfg!(target_os = "macos") {
        ...
        if ext.eq_ignore_ascii_case("ogg") {
            ...
            if which_command("paplay").is_ok() {
                ...
                tracing::warn!(target: "agent_of_empires", "paplay not found, using aplay (may not support .ogg files)");
...
pub fn play_sound(name: &str, volume: f64) {
    let Some(path) = find_sound_file(name) else {
        tracing::debug!(target: "agent_of_empires", "Sound file not found: {}", name);
        return;
    };
    ...
    std::thread::spawn(move || {
        let (cmd, args) = match get_audio_command(&path_str, volume) {
            Ok(result) => result,
            Err(e) => {
                tracing::warn!(target: "agent_of_empires", "Audio player not available: {}", e);
                return;
            }
        };
        ...
        if let Err(e) = result {
            tracing::debug!(target: "agent_of_empires", "Failed to play sound: {}", e);
        }
    });
}
