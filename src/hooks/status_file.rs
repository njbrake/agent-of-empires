...
pub fn read_hook_status(instance_id: &str) -> Option<Status> {
    ...
    match content.trim() {
        ...
        other => {
            tracing::warn!(target: "agent_of_empires", "Unexpected hook status value: {:?}", other);
            None
        }
    }
}
...
pub fn cleanup_hook_status_dir(instance_id: &str) {
    ...
    if dir.exists() {
        if let Err(e) = std::fs::remove_dir_all(&dir) {
            tracing::warn!(target: "agent_of_empires", "Failed to cleanup hook status dir {}: {}", dir.display(), e);
        }
    }
}
