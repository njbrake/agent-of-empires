...
pub fn install_hooks(settings_path: &Path, events: &[crate::agents::HookEvent]) -> Result<()> {
    let mut settings: Value = if settings_path.exists() {
        ...
        serde_json::from_str(&content).unwrap_or_else(|e| {
            tracing::warn!(target: "agent_of_empires", "Failed to parse {}: {}", settings_path.display(), e);
            serde_json::json!({})
        })
    } else {
        serde_json::json!({})
    ...
    tracing::info!(target: "agent_of_empires", "Installed AoE hooks in {}", settings_path.display());
    Ok(())
}
...
pub fn uninstall_hooks(settings_path: &Path) -> Result<bool> {
    ...
    tracing::info!(target: "agent_of_empires", "Removed AoE hooks from {}", settings_path.display());
    Ok(true)
}
...
pub fn install_settl_hooks() -> Result<()> {
    ...
    let mut config: toml::Value = if config_path.exists() {
        ...
        toml::from_str(&content).unwrap_or_else(|e| {
            tracing::warn!(target: "agent_of_empires", "Failed to parse {}: {}", config_path.display(), e);
    ...
    tracing::info!(target: "agent_of_empires", "Installed AoE hooks in {}", config_path.display());
    Ok(())
}
...
pub fn uninstall_settl_hooks() -> Result<bool> {
    ...
    let formatted = toml::to_string_pretty(&config)?;
    std::fs::write(&config_path, formatted)?;
    tracing::info!(target: "agent_of_empires", "Removed AoE hooks from {}", config_path.display());
    Ok(true)
}
...
pub fn install_kiro_hooks(agent_config_path: &Path) -> Result<()> {
    ...
    tracing::info!(target: "agent_of_empires", "Installed AoE hooks in {}", agent_config_path.display());
    Ok(())
}
    ...
    if is_builtin_default {
        ...
        match set_result {
            Ok(o) if o.status.success() => {
                tracing::info!(target: "agent_of_empires", "Set aoe-hooks as default Kiro agent for status detection");
            }
            Ok(o) => {
                tracing::debug!(target: "agent_of_empires",
                    "kiro-cli agent set-default failed (non-fatal): {}",
                    String::from_utf8_lossy(&o.stderr)
                );
            }
            Err(e) => {
                tracing::debug!(target: "agent_of_empires", "kiro-cli not available for set-default: {}", e);
            }
        }
    } else {
        tracing::info!(target: "agent_of_empires",
...
pub fn uninstall_kiro_hooks(agent_config_path: &Path) -> Result<bool> {
    ...
    tracing::info!(target: "agent_of_empires", "Removed AoE hooks from {}", agent_config_path.display());
    Ok(true)
}
...
pub fn uninstall_all_hooks() {
    ...
    match uninstall_settl_hooks() {
        ...
        Ok(false) => {}
        Err(e) => tracing::warn!(target: "agent_of_empires", "Failed to remove settl hooks: {}", e),
    }
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        match uninstall_hermes_hooks(&hermes_config) {
            ...
            Ok(false) => {}
            Err(e) => {
                tracing::warn!(target: "agent_of_empires", "Failed to remove hermes hooks: {}", e)
            }
        }
        ...
        match uninstall_kiro_hooks(&kiro_config) {
            ...
            Ok(false) => {}
            Err(e) => {
                tracing::warn!(target: "agent_of_empires", "Failed to remove kiro hooks: {}", e)
            }
        }
        ...
        for agent in crate::agents::AGENTS {
            if let Some(hook_cfg) = &agent.hook_config {
                ...
                match uninstall_hooks(&settings_path) {
                    ...
                    Ok(false) => {}
                    Err(e) => {
                        tracing::warn!(target: "agent_of_empires",
    ...
    if base.exists() {
        if let Err(e) = std::fs::remove_dir_all(base) {
            tracing::warn!(target: "agent_of_empires", "Failed to remove {}: {}", base.display(), e);
        }
    }
}
